...
void Twins(int band, int barHeight)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    ...
    for (int y = TOP; y >= 2; y--)
    {
      ...
      {
        if ((band & 1) == 1)
          dma2_display->drawPixelRGB888(x, y, 250, 0, 0);
        else
          dma2_display->drawPixelRGB888(x, y, 250, 250, 0); // middle
      }
      else
      {
        // leds[i].fadeToBlackBy( 64 );
...
void Twins2(int band, int barHeight)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    ...
    for (int y = TOP; y >= 2; y--)
    {
      ...
      {
        if ((band & 1) == 1)
          dma2_display->drawPixelRGB888(x, y, 250, 0, 250);
        else
          dma2_display->drawPixelRGB888(x, y, 0, 250, 250); // middle
      }
      else
      {
        // leds[i].fadeToBlackBy( 64 );
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    ...
    for (int y = TOP; y >= 2; y--)
    {
      if (y >= TOP - barHeight)
      {
        ...
        if (y < (PANEL_HEIGHT / 4))
          dma2_display->drawPixelRGB888(x, y, TriBar_RGB_Top); // Top
        else if (y > (PANEL_HEIGHT / 2))
          dma2_display->drawPixelRGB888(x, y, TriBar_RGB_Bottom); // bottom
        else
          dma2_display->drawPixelRGB888(x, y, TriBar_RGB_Middle); // middle
    ...
    for (int y = TOP; y >= 2; y--)
    {
      if (y >= TOP - barHeight)
      {
        ...
        if (y == (TOP - barHeight))
          ...
          dma2_display->drawPixelRGB888(x, y, 250, 0, 0); // Border left side of the bars
        else if (x == xStart + BAR_WIDTH - 1)
          dma2_display->drawPixelRGB888(x, y, 250, 0, 0); // Border right side of the bars
        else
          dma2_display->drawPixelRGB888(x, y, 0, 0, 250);
      }
      else
      {
        // leds[i].fadeToBlackBy( 64 );
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    for (int y = TOP; y >= 2; y--)
    {
      if (y >= TOP - barHeight)
      {
        ...
        if (y == (TOP - barHeight))
          dma2_display->drawPixelRGB888(x, y, 250, 250, 250);
        else if (x == xStart)
          dma2_display->drawPixelRGB888(x, y, 250, 250, 250); // Border left side of the bars
        else if (x == xStart + BAR_WIDTH - 1)
          dma2_display->drawPixelRGB888(x, y, 250, 250, 250); // Border right side of the bars
        else
          dma2_display->drawPixelRGB888(x, y, 0, 0, 250);
      }
      else
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    for (int y = TOP; y >= 2; y--)
    {
      if (y >= TOP - barHeight)
      {
        ...
        if (y == (TOP - barHeight))
          dma2_display->drawPixelRGB888(x, y, 0, 255, 0);
        else if (x == xStart)
          dma2_display->drawPixelRGB888(x, y, 0, 255, 0); // Border left side of the bars
        else if (x == xStart + BAR_WIDTH - 1)
          dma2_display->drawPixelRGB888(x, y, 0, 255, 0); // Border right side of the bars
        else
          dma2_display->drawPixelRGB888(x, y, 200, 200, 0);
      }
      else
      {
        // leds[i].fadeToBlackBy( 64 );
...
void centerBars(int band, int barHeight)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    ...
    for (int y = barHeight / 2; y < TOP; y++)
    {
      ...
      if ((center - y - 2) > 1)
        dma2_display->drawPixelRGB888(x, center - y - 2, 0, 0, 0); // make unused pixel top black except those of the VU meter
    }
  }
}
...
void TriPeak(int band)
{
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++)
  {
    ...
    if (peakHeight < (PANEL_HEIGHT / 4))
      dma2_display->drawPixelRGB888(x, peakHeight, TriBar_RGB_Top); // Top red
    else if (peakHeight > (PANEL_HEIGHT / 2))
      dma2_display->drawPixelRGB888(x, peakHeight, TriBar_RGB_Bottom); // green
    else
      dma2_display->drawPixelRGB888(x, peakHeight, TriBar_RGB_Middle); // yellow
  }
}
