...
Clockface_pokedex::Clockface_pokedex(Adafruit_GFX* display)
{
  _display = display;
  Locator::provide(display);
}

void Clockface_pokedex::setup(CWDateTime *dateTime) {
...
void Clockface_pokedex::refreshDate(uint8_t weekday, uint16_t color) {
  // Update weekday
...
void Clockface_pokedex::refreshTime() { 
...
void Clockface_pokedex::updatePokemon() { 
  Locator::getDisplay()->drawRGBBitmap(8, 21, pokemons[random(sizeof(pokemons)/4)], 16, 16);
}

void Clockface_pokedex::updateLoadingBar(uint8_t seconds) {
