...
static unsigned long lastMillis = millis() + 60000;
...
Clockface_castlevania::Clockface_castlevania(Adafruit_GFX* display) {
  _display = display;
  Locator::provide(display);
}

void Clockface_castlevania::setup(CWDateTime *dateTime) {
  this->_dateTime = dateTime;
  Locator::getDisplay()->fillRect(0, 0, 64, 64, 0x0000);  
  updateClock();
}

void Clockface_castlevania::update()
{ 
...
void Clockface_castlevania::drawClockPointer(float angle, byte pointerSize, uint16_t color) 
{
...
float Clockface_castlevania::degreesToRadians(float degrees)
{
    return (degrees * 3.14) / 180;
}

void Clockface_castlevania::updateClock() 
{
