...
Clockface_worldMap::Clockface_worldMap(Adafruit_GFX* display) {
  _display = display;

  Locator::provide(display);
}

void Clockface_worldMap::setup(CWDateTime *dateTime) {
...
void Clockface_worldMap::update() 
{  
  ...
  if (millis() - lastMillis >= 1000) { 
    ...
    Locator::getDisplay()->setFont(&small4pt7b_worldMap);  
...
void Clockface_worldMap::updateMap() 
{
...
void Clockface_worldMap::croppedDraw(const unsigned short* image_array, int x, int y, int anchorX, int anchorY, int cropX, int cropY, int w, int h)
{  
  int anchor = anchorX;
