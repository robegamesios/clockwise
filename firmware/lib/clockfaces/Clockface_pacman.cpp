...
Clockface_pacman::Clockface_pacman(Adafruit_GFX* display) {
  _display = display;
  Locator::provide(display);
}

void Clockface_pacman::setup(CWDateTime *dateTime) {
...
const char* Clockface_pacman::weekDayName(int weekday) {
  strncpy(weekDayTemp, _weekDayWords + (weekday*4), 4);
  return weekDayTemp;
}

const char* Clockface_pacman::monthName(int month) {
...
void Clockface_pacman::updateClock() {
...
void Clockface_pacman::directionDecision(MapBlock nextBlk, bool moving_axis_x) {
...
void Clockface_pacman::resetMap() {
...
int Clockface_pacman::countBlocks(Clockface_pacman::MapBlock elem) {
  int count = 0;
...
void Clockface_pacman::turnRandom() {
  int dir = random(4);
  //int dir = 3;
...
Clockface_pacman::MapBlock Clockface_pacman::nextBlock() {
  return nextBlock(pacman->_direction);
}

Clockface_pacman::MapBlock Clockface_pacman::nextBlock(Direction dir) {

  Clockface_pacman::MapBlock map_block = Clockface_pacman::MapBlock::OUT_OF_MAP;
...
bool Clockface_pacman::contains(int v, const int* values) {
